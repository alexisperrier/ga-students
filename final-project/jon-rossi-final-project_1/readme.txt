...
In terminal, run python twitter_search.py > ‘twitter_search_results.txt’ to get text file of twitter results. (Need config file with authentication keys, which is not included).
...
** yankees-analysis.ipynb is an analogous notebook to redsox-analysis.ipynb but does not yet include the Predicting Wins section, with regression and classification attempts. The same methods would be applied to both notebooks, but the attempts on the Redsox data weren’t successful enough.
